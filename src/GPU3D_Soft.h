...
namespace GPU3D
{
    ...
    private:
        ...
        public:
            ...
            s32 Setup(s32 x0, s32 x1, s32 y0, s32 y1, s32 w0, s32 w1, s32 y)
            {
                ...
                if (side)
                {
                    ...
                    if (XMajor)
                        dx = Negative ? (0x20000 + 0x40000) : (Increment - 0x20000);
                    else if (Increment != 0)
                        dx = Negative ? 0x40000 : 0;
                    else
                        dx = -0x40000;
                }
                else
                {
                    ...
                    if (XMajor)
                        dx = Negative ? ((Increment - 0x20000) + 0x40000) : 0x20000;
                    else if (Increment != 0)
                        dx = Negative ? 0x40000 : 0;
                    else
                        dx = 0;
                ...
                {
                    if (side)
                        Interp.Setup(x0 - 1, x1 - 1, w0, w1); // checkme
                    else
        ...
        void ClearBuffers();
        void RenderPolygons(Polygon **polygons, int npolys);
