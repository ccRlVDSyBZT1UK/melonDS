...
#define INSTANTIATE_SLOWMEM(consoleType)                                                 \
                      ...
                      u32 &linkAddr, u32 &targetAddr)
    {
        ...
        else
        {
            ...
            cond = instr.Cond();
            if (instr.Info.Kind == ARMInstrInfo::ak_BL || instr.Info.Kind == ARMInstrInfo::ak_B)
            {
...
#undef F
    ...
    void CompileBlock(ARM *cpu)
    {
        ...
        do
        {
            ...
            if (instrs[i].Info.DstRegs & (1 << 14) || (!thumb && (instrs[i].Info.Kind == ARMInstrInfo::ak_MSR_IMM || instrs[i].Info.Kind == ARMInstrInfo::ak_MSR_REG) && instrs[i].Instr & (1 << 16)))
                hasLink = false;
            ...
            else
            {
                ...
                else
                {
                    ...
                    assert(InterpretARM[instrs[i].Info.Kind] == ARMInterpreter::ARMInstrTable[icode] || instrs[i].Info.Kind == ARMInstrInfo::ak_MOV_REG_LSL_IMM || instrs[i].Info.Kind == ARMInstrInfo::ak_Nop || instrs[i].Info.Kind == ARMInstrInfo::ak_UNK);
            ...
            u32 literalAddr;
            if (LiteralOptimizations && instrs[i].Info.SpecialKind == ARMInstrInfo::special_LoadLiteral && DecodeLiteral(thumb, instrs[i], literalAddr))
            {
            ...
            else if (thumb && instrs[i].Info.Kind == ARMInstrInfo::tk_BL_LONG_2 && i > 0 && instrs[i - 1].Info.Kind == ARMInstrInfo::tk_BL_LONG_1)
            {
                i--;
            ...
            if (instrs[i].Info.Branches() && BranchOptimizations && instrs[i].Info.Kind != (thumb ? ARMInstrInfo::tk_SVC : ARMInstrInfo::ak_SVC))
            {
        ...
        if (prevBlockIt != RestoreCandidates.end())
        {
            ...
            if (mayRestore && prevBlock->NumAddresses == numAddressRanges)
            {
                for (u32 j = 0; j < numAddressRanges; j++)
                {
                    if (prevBlock->AddressRanges()[j] != addressRanges[j] || prevBlock->AddressMasks()[j] != addressMasks[j])
                    {
                        mayRestore = false;
                        break;
                    }
        ...
        if (!mayRestore)
        {
            ...
            JIT_DEBUGPRINT("block entry point %d\n", (int)block->EntryPoint);
        }
        else
        {
    ...
    void InvalidateByAddr(u32 localAddr)
    {
        ...
        for (int i = 0; i < range->Blocks.Length;)
        {
            ...
            if (range->Blocks.Length == 0 && !PageContainsCode(&region[(localAddr & 0x7FFF000) / 512]))
            {
    ...
    bool SetupExecutableRegion(u32 num, u32 blockAddr, u64 *&entry, u32 &start, u32 &size)
    {
        ...
        u32 memoryOffset;
        if (FastBlockLookupRegions[region] && ARMJIT_Memory::GetMirrorLocation(region, num, blockAddr, memoryOffset, start, size))
        {
