...
namespace wasmblr
{
    ...
    struct Import
    {
        Import(std::string module_name_,
               std::string field_name_,
               uint8_t import_desc_,
               uint32_t limit_min_,
               uint32_t limit_max_)
            : module_name(module_name_), field_name(field_name_), import_desc(import_desc_), limit_min(limit_min_), limit_max(limit_max_) {}
    ...
    struct CodeGenerator
    {
        ...
        void call_indirect(uint32_t typeidx);
        void import_table(std::string module, std::string field, uint32_t min, uint32_t max);
        void import_mem(std::string module, std::string field, uint32_t min, uint32_t max);
...
#define VECTOR_UNARY_OP(op, vopcode, inp_type, out_type) \
    ...
    inline void CodeGenerator::call_indirect(uint32_t type_idx)
    {
        emit(0x11);
        emit(encode_unsigned(type_idx));
        emit(encode_unsigned(0));
    }
    ...
    inline void CodeGenerator::import_table(std::string module, std::string field, uint32_t min, uint32_t max)
    {
        imports_.emplace_back(module, field, 0x01, min, max);
    }

    inline void CodeGenerator::import_mem(std::string module, std::string field, uint32_t min, uint32_t max)
    {
        imports_.emplace_back(module, field, 0x02, min, max);
    }
    ...
    inline std::vector<uint8_t> CodeGenerator::emit()
    {
        ...
        concat(import_section_bytes, encode_unsigned(imports_.size()));
        for (const auto &import : imports_)
        {
            concat(import_section_bytes, encode_string(import.module_name));
            concat(import_section_bytes, encode_string(import.field_name));
            import_section_bytes.emplace_back(import.import_desc); // table type
            if (import.import_desc == 0x01)
            {
                import_section_bytes.emplace_back(0x70); // function reference
            }
            if (import.limit_max < import.limit_min)
            {
                import_section_bytes.emplace_back(0x0); // [min, +inf]
                concat(import_section_bytes, encode_unsigned(import.limit_min));
            }
            else
            {
                import_section_bytes.emplace_back(0x1); // [min, max]
                concat(import_section_bytes, encode_unsigned(import.limit_min));
                concat(import_section_bytes, encode_unsigned(import.limit_max));
            }
        }
