...
namespace GPU3D
{
    ...
    void SoftRenderer::RenderPolygonScanline(RendererPolygon *rp, s32 y)
    {
        ...
        if (!l_filledge)
            x = std::min(xlimit, xend - r_edgelen + 1);
        else
            for (; x < xlimit; x++)
            {
                u32 pixeladdr = FirstPixelOffset + (y * ScanlineWidth) + x;
    ...
    u32 SoftRenderer::CalculateFogDensity(u32 pixeladdr)
    {
        ...
        u32 density =
            ((RenderFogDensityTable[densityid] * (0x20000 - densityfrac)) +
             (RenderFogDensityTable[densityid + 1] * densityfrac)) >>
            17;
        if (density >= 127)
            density = 128;
    ...
    void SoftRenderer::ScanlineFinalPass(s32 y)
    {
        ...
        if (RenderDispCnt & (1 << 5))
        {
            ...
            for (int x = 0; x < 256; x++)
            {
                    ...
                    ((polyid != (AttrBuffer[pixeladdr + ScanlineWidth] >> 24)) && (z < DepthBuffer[pixeladdr + ScanlineWidth])))
                {
                    ...
                    u32 edgeR = (edgecolor << 1) & 0x3E;
                    if (edgeR)
                        edgeR++;
                    u32 edgeG = (edgecolor >> 4) & 0x3E;
                    if (edgeG)
                        edgeG++;
                    u32 edgeB = (edgecolor >> 9) & 0x3E;
                    if (edgeB)
                        edgeB++;
        ...
        if (RenderDispCnt & (1 << 7))
        {
            ...
            u32 fogR = (RenderFogColor << 1) & 0x3E;
            if (fogR)
                fogR++;
            u32 fogG = (RenderFogColor >> 4) & 0x3E;
            if (fogG)
                fogG++;
            u32 fogB = (RenderFogColor >> 9) & 0x3E;
            if (fogB)
                fogB++;
    ...
    void SoftRenderer::ClearBuffers()
    {
        ...
        {
            // TODO: confirm color conversion
            u32 r = (RenderClearAttr1 << 1) & 0x3E;
            if (r)
                r++;
            u32 g = (RenderClearAttr1 >> 4) & 0x3E;
            if (g)
                g++;
            u32 b = (RenderClearAttr1 >> 9) & 0x3E;
            if (b)
                b++;
    ...
    void SoftRenderer::RenderPolygons(Polygon **polygons, int npolys)
    {
        int j = 0;
        for (int i = 0; i < npolys; i++)
        {
            if (polygons[i]->Degenerate)
                continue;
    ...
    void SoftRenderer::RenderFrame()
    {
        ...
        {
            ClearBuffers();
            RenderPolygons(&RenderPolygonRAM[0], RenderNumPolygons);
        }
    }
