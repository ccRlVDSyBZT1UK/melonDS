...
*/
    ...
    bool Init()
    {
        ...
        Framebuffer[0][1] = NULL;
        Framebuffer[1][0] = NULL;
        Framebuffer[1][1] = NULL;
        Renderer = 0;
...
#define MAP_RANGE(map, base, n) \
    for (int i = 0; i < n; i++) \
        VRAMMap_##map[(base) + i] |= bankmask;
#define UNMAP_RANGE(map, base, n) \
    for (int i = 0; i < n; i++)   \
        VRAMMap_##map[(base) + i] &= ~bankmask;
...
#define MAP_RANGE_PTR(map, base, n)                                                                  \
    ...
    {                                                                                                \
        VRAMMap_##map[(base) + i] |= bankmask;                                                       \
        VRAMPtr_##map[(base) + i] = GetUniqueBankPtr(VRAMMap_##map[(base) + i], ((base) + i) << 14); \
    }
#define UNMAP_RANGE_PTR(map, base, n)                                                                \
    ...
    void SetPowerCnt(u32 val)
    {
        ...
        if (!(val & (1 << 0)))
            printf("!!! CLEARING POWCNT BIT0. DANGER\n");
    ...
    void StartHBlank(u32 line)
    {
        ...
        if (DispStat[0] & (1 << 4))
            NDS::SetIRQ(0, NDS::IRQ_HBlank);
        if (DispStat[1] & (1 << 4))
            NDS::SetIRQ(1, NDS::IRQ_HBlank);
    ...
    void StartScanline(u32 line)
    {
        ...
        else
        {
            if (VCount == 192)
            {
                ...
                if (DispStat[0] & (1 << 3))
                    NDS::SetIRQ(0, NDS::IRQ_VBlank);
                if (DispStat[1] & (1 << 3))
                    NDS::SetIRQ(1, NDS::IRQ_VBlank);
    ...
    T ReadVRAM_ABGExtPal(u32 addr)
    {
        ...
        T ret = 0;
        if (mask & (1 << 4))
            ret |= *(T *)&VRAM_E[addr & 0x7FFF];
        if (mask & (1 << 5))
            ret |= *(T *)&VRAM_F[addr & 0x3FFF];
        if (mask & (1 << 6))
            ret |= *(T *)&VRAM_G[addr & 0x3FFF];
    ...
    T ReadVRAM_AOBJExtPal(u32 addr)
    {
        ...
        T ret = 0;
        if (mask & (1 << 4))
            ret |= *(T *)&VRAM_F[addr & 0x1FFF];
        if (mask & (1 << 5))
            ret |= *(T *)&VRAM_G[addr & 0x1FFF];
